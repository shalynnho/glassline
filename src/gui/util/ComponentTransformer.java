...
public class ComponentTransformer extends JPanel {
	/**
	 * this is glass panel taking care of transforming click so that transformed label takes clicks on it not on the previous location
	 */
	 ...
	 * 
	 * @param view is the component which will get transformed inside the panel
	 */
	...
	/**
	 * This constructor gets the component and transform if user wants to pass his own transform object otherwise it creates a new instance of affine transform
	 * 
	 * @param view
	 * @param at
	 */
	...
	/**
	 * sets the location of the glass panel that takes care of click transformation to 0,0 in the main panel it also sets the location of the view in the visible triangle of the panel so view is
	 * always visible it also sets the size of the panel to be the same as the view so that it would take least amount of space
	 */
	...
	/**
	 * returns the new size of the transformed component it is the rectangle that holds the transformed component in it it make sure that the boundaries are taken into account
	 */
	...
	/**
	 * overriding the paint method so it would draw the transformed graphics instead of original graphics of the component;
	 */
	...
	private class MagicGlassPane extends JPanel {
		...
		/**
		 * default constructor it makes the panel transparent and enables all mouse events on it so that it will receive all mouse events even if it does not have listener
		 */
		...
		private MouseEvent transformMouseEvent(MouseEvent event) {
			...
			if (view != null && at.getDeterminant() != 0) {
				...
				mouseCurrentComponent = SwingUtilities.getDeepestComponentAt(view, (int) tp.getX(), (int) tp.getY());
		...
		/**
		 * function that dispatches the new created event with transformed elements to all the listeners that listens to the first created event
		 * 
		 * @param event
		 */
		...
		protected void processMouseWheelEvent(MouseWheelEvent e) {
			...
			MouseWheelListener[] mouseWheelListeners = transformedEvent.getComponent().getMouseWheelListeners();
		...
		private MouseEvent createEnterExitEvent(Component c, int eventId, MouseEvent mouseEvent) {
			return new MouseEvent(c, eventId, mouseEvent.getWhen(), 0, mouseEvent.getX(), mouseEvent.getY(), 0, false, MouseEvent.NOBUTTON);
		}
	...
	/**
	 * This class helps view component to be in the visible area; this is important when transformer is inside JScrollPane
	 */
	...
	/**
	 * Never returns null it just creates a new affinetransform and returns
	 */
	...
	/**
	 * sets the new received transform to the respective field variable so the paint method can use the new transform and paint
	 * 
	 * @param at new transform for teh view
	 */
	 ...
	 * 
	 * @param theta degrees for turning in radians
	 */
	 ...
	 * 
	 * @param sx new x dimension
	 * @param sy new y dimension
	 */
	 ...
	 * 
	 * @param x the new x location for the center
	 * @param y the new y location for the center
	 */
	...
	public Polygon getTransformedBounds() {
		...
		if (view != null && at.getDeterminant() != 0) {
			...
			return new Polygon(new int[] { (int) (first.getX() + tx + getX()), (int) (second.getX() + tx + getX()), (int) (third.getX() + tx + getX()), (int) (forth.getX() + tx + getX()) }, new int[] { (int) (first.getY() + ty + getY()), (int) (second.getY() + ty + getY()), (int) (third.getY() + ty + getY()), (int) (forth.getY() + ty + getY()) }, 4);
		} else
			return null;
	}
	...
	/**
	 * makes sure the component gets transformed with keeping the center of the panel fixed so transformation wont happen around the top left corner of the panel
	 */
