...
public class ImageIconUtil {
	...
	/**
	 * it gets a string which is is a path to a specific image. and returns an image icon with that image and passed dimension
	 * 
	 * @param path path to the image file
	 * @param width required width of the ImageIcon
	 * @param height required height of the ImageIcon
	 * @return new Image Icon created
	 */
	public static ImageIcon getImageIcon(String path, int width, int height) {
		try {
			return new ImageIcon(ImageIO.read(new File(path)).getScaledInstance(width, height, java.awt.Image.SCALE_SMOOTH));
		} catch (IOException e) {
