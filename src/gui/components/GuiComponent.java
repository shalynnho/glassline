...
/**
 * The GuiComponent class is the superclass for all components in the factory GUI. It provides methods to change the location of the component, and to update the component. Subcomponents must override
 * the update method with their own function.
 */
...
public abstract class GuiComponent extends JLabel implements ActionListener, TReceiver {
	 ...
	 * 
	 * @param dPanel the DisplayPanel linked to this panel
	 */
	...
	/**
	 * Moves this component to a new location. The middle of the new location is specified by the x and y parameters in the coordinate space of this component's parent.
	 * 
	 * @param x the x-coordinate of the new location's middle point in the parent's coordinate space
	 * @param y the y-coordinate of the new location's middle point in the parent's coordinate space
	 */
	...
	/**
	 * Moves this component to a new location. The middle point of the new location is specified by the x and y parameters in the coordinate space of this component's parent.
	 * 
	 * @param p the point defining the middle point of the new location, given in the coordinate space of this component's parent
	 */
	...
	/**
	 * Gets the location of this component in the form of a point specifying the component's middle point. The location will be relative to the parent's coordinate space.
	 * 
	 * @return an instance of Point representing the middle point of the component's bounds in the coordinate space of the component's parent
	 */
	public final Point getCenterLocation() {
		if (transformer == null)
			return (new Point(getX() + getWidth() / 2, getY() + getHeight() / 2));
		else
			return (new Point(transformer.getX() + transformer.getWidth() / 2, transformer.getY() + transformer.getHeight() / 2));

	}

	/**
	 * Moves this component relative to its current location a specified distance. Keep in mind that this is relative movement!
	 * 
	 * @param dx the horizontal distance to move the component. To move left, dx should be negative, to move right, positive
	 * @param dy the vertical distance to move the component. To move up, dy should be negative, to move down, positive
	 */
	...
	public Polygon getPolyBounds() {
		if (transformer == null) {
			return new Polygon(new int[] { getX(), getX() + getWidth(), getX(), getX() + getHeight() }, new int[] { getY(), getY(), getY() + getHeight(), getY() + getHeight() }, 4);
		} else
	...
	/**
	 * Called every time the gui Timer is fired. Updates the Component graphically according to its function. Subclasses must implement this method with their own animations.
	 */
	@Override
	public abstract void actionPerformed(ActionEvent ae);

	/**
	 * Called every time the transducer is fired. Corresponds to an Agent command, to which the component should respond.
	 */
	@Override
