...
/**
 * GUIBin is a graphical representation of the bin component that holds the parts delivered by the trucks.
 */
...
public class GUIBin extends GuiComponent {
	 ...
	 * 
	 * @param bin reference to the bin that GUIBin is created in
	 */
	...
	public void addNextComponent(GuiComponent component) {
		...
		target = new Point(nextComponent.getCenterX() + (2 * nextComponent.getIcon().getIconWidth()) / 5, nextComponent.getCenterY());
	}
	...
	public void eventFired(TChannel channel, TEvent event, Object[] args) {
		if (event == TEvent.BIN_CREATE_PART) {
			...
			parent.getActivePieces().add(part); // inherits parent from GuiComponent
