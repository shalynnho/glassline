...
/**
 * The StatePanel controls the factory's run state and the speed of the factory. It can be extended to control music, etc.
 */
...
public class StatePanel extends JPanel {
	 ...
	 * 
	 * @param cPanel ControlPanel to link this panel to
	 * @param intialPause whether the initial state of the factory is paused
	 */
	public StatePanel(ControlPanel cPanel, boolean initialPause) {
		...
		int initialSliderValue = (MAX_SLIDER_VALUE - MIN_SLIDER_VALUE) * (FactoryFrame.DEFAULT_FRAMES_PER_SECOND - MIN_FRAMES_PER_SECOND) / (MAX_FRAMES_PER_SECOND - MIN_FRAMES_PER_SECOND) + MIN_SLIDER_VALUE + 1;
		...
		debugButton.setText("DEBUG: " + FactoryFrame.DEFAULT_FRAMES_PER_SECOND);
	 ...
	 * 
	 * @param cPanel the ControlPanel to link this panel to
	 */
	...
	/**
	 * Listener class for the debug button NOTE: the button does not actually get this class. Rather, it is called every timestep to update the button
	 */
	...
	public class StopButtonListener implements ActionListener {
		/**
		 * Invoked whenever the button is clicked, starts the control cell. Note that this button is disabled unless the factory is running
		 */
		public void actionPerformed(ActionEvent ae) {
	...
	public class SpeedSliderListener implements ChangeListener {
		/**
		 * Invoked whenever the timer speed slider is changed, updates the factory speed. Note that lower on the slider is slower factory
		 */
		public void stateChanged(ChangeEvent ce) {
			int newSpeed = (speedSlider.getValue() * (MAX_FRAMES_PER_SECOND - MIN_FRAMES_PER_SECOND) / (MAX_SLIDER_VALUE - MIN_SLIDER_VALUE) + MIN_FRAMES_PER_SECOND);
